...
class progress_report_format_email{
     ...
     */  
    const SETTING_ENABLED                = 'grade_report_progress_email_enabled';
    ...
    public function render_content($formatted, $user) {
        ...
        $status = get_content_status($sent, $recipients);
        ...
        $formattedcontent = $formatter->format_content($status);
    ...
    static public function available($courseid) {
        //**should also check if student and students are allowed (see the viewall permission...
        //get a setting
    ...
    public function get_content_status($sent, $recipients) {
        ...
        $item['children'][0]['children'][0] = $prg::build_content_node($text);
        ...
        //dl
        $item['children'][1] = $prg::build_content_container_node($prg::HTML_DEFINITION_LIST);
        $i = 0;
        
        foreach ($recipients as $id => $recipient) {
            //dt - username
            $text = fullname($recipient);
            $item['children'][1]['children'][$i] = $prg::build_content_container_node($prg::HTML_DEFINITION_TERM);
            ...
            $item['children'][1]['children'][$i] = $prg::build_content_container_node($prg::HTML_DEFINITION_DEFINITION);
            $item['children'][1]['children'][$i]['children'][0] = $prg::build_content_text_node($text);
            $i++;
        }
