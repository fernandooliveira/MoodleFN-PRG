...
class progress_report_subreport_summary{
     ...
     */
    const SETTING_ENABLED                       = 'grade_report_progress_sum_enabled';
    /**
     * consumed by grade_get_setting
     * @const string SETTING_WEIGHT
     */
    const SETTING_WEIGHT                        = 'grade_report_progress_sum_weight';
   ...
   /**
    ...
    protected function initialize_attendance() {
        ...
        $params = array($this->userid, $this->courseid);
        $sql = "SELECT time FROM {log} " .
               "WHERE  userid = ? AND course = ? " .
               "ORDER BY time ASC";
        $this->hourslogged = 0;
        if ($visits = $DB->get_record_sql($sql, $params)) {
