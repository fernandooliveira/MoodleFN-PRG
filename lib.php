...
//include format class files
$prg_subplugin_manager_instance = progress_report_subplugin_manager::get_instance();
if ($files = $prg_subplugin_manager_instance->get_subplugin_files('formats','format.class.php')) {
    foreach ($files as $file) {
        require_once($file);
    }
}
//include report class files
if ($files = $prg_subplugin_manager_instance->get_subplugin_files('reports','report.class.php')) {
    foreach ($files as $file) {
        require_once($file);
    }
}
...
//build course settings page from mform pages of subplugins
function grade_report_progress_settings_definition(&$mform) {
    global $CFG;
    
    $prg_subplugin_manager_instance = progress_report_subplugin_manager::get_instance();
    
    //foreach setting in normal, add a setting to the mform
    //static
    $mform->addElement('static', 'grade_report_progress_core_heading', get_string('settingprogressname', 'gradereport_progress' ), get_string('settingprogressfulldescription', 'gradereport_progress' ));
    //selectyesno
    $mform->addElement('selectyesno', 'report_grade_progress_allow_students', get_string('settingallowstudents', 'gradereport_progress' ));
    $mform->addHelpButton('report_grade_progress_allow_students', 'settingallowstudents', 'gradereport_progress');
    
    //static
    $mform->addElement('html', '<hr style="border:solid;color:#CCC;">');
    $mform->addElement('static', 'grade_report_progress_reports_heading', get_string('settingreportsheader', 'gradereport_progress' ), get_string('settingreportsheaderfulldescription', 'gradereport_progress' ));
    $mform->addElement('html', '<hr style="border:solid;color:#CCC;">');
    //include files for reports
    if ($files = $prg_subplugin_manager_instance->get_subplugin_files('reports','settings.mform.php')) {
        foreach ($files as $file) {
    ...
    //static
    $mform->addElement('html', '<hr style="border:solid;color:#CCC;">');
    $mform->addElement('static', 'grade_report_progress_reports_heading', get_string('settingformatssheader', 'gradereport_progress' ), get_string('settingformatssheaderfulldescription', 'gradereport_progress' ));
    $mform->addElement('html', '<hr style="border:solid;color:#CCC;">');
    //include files for formats
    if ($files = $prg_subplugin_manager_instance->get_subplugin_files('formats','settings.mform.php')) {
        foreach ($files as $file) {
            $mform->addElement('html', '<hr>');
            require_once($file);
        }
...
class progress_report_generator {                 
    ...
    public function __construct($opt) {
        ...
        $prg_subplugin_manager_instance = progress_report_subplugin_manager::get_instance();
        $this->allowedsubreports = $prg_subplugin_manager_instance->reports;
        $this->allowedformats = $prg_subplugin_manager_instance->formats;
    }
    ...
    public function get_selectable_users($courseid, $userid = null) {
        ...
        if (!has_capability('gradereport/progress:viewall', $coursecontext)){ 
            ...
            $defaultvalue = isset($CFG->report_grade_progress_allow_students)? $CFG->report_grade_progress_allow_students : 0;
        
            if (grade_get_setting($courseid, 'report_grade_progress_allow_students', $defaultvalue, false)) {
                $samecoursesql =  "JOIN 
                                  (
                                  SELECT u1.id 
                                  FROM {user} u1 
    ...
    public function get_content_report_select_ui() {
        ...
        //subreport weight boxes - feature removed
        /*
