...
class progress_report_subreport_activity{
     ...
     */
    const SETTING_ENABLED                = 'report_grade_progress_act_enabled';
    /**
     * consumed by grade_get_setting
     * @const string SETTING_WEIGHT
     */
    const SETTING_WEIGHT                 = 'report_grade_progress_act_weight';

    /**
     * consumed by grade_get_setting
     * @const string SETTING_DEFAULT_SORT
     */
    const SETTING_DEFAULT_SORT           = 'report_grade_progress_act_sort_default';
    ...
    public function __construct($userid, $courseid, $context, $course, $params) {
        ...
        if (!$sortfound) {
            if ($courseid == -1) {
                $defaultsort = self::SORT_COURSE;
            }
            else {
                $defaultsort = self::get_default_sort($courseid);
            }
            $this->sort = optional_param('actsort', $defaultsort, PARAM_INT);
    ...
    /**
     * Determines the default sort of this subreport
     * @return int default sort
     */
    static public function get_default_sort($courseid) {
        global $CFG;

        $classsort = self::SETTING_DEFAULT_SORT;

        $defaultvalue = isset($CFG->$classsort)? $CFG->$classsort : self::SORT_COURSE;
        return grade_get_setting($courseid, $classsort, $defaultvalue);
    }
    ...
    static public function get_params($weight) {
        ...
        $params = array();
        $sort = optional_param('actsort', self::SORT_COURSE, PARAM_INT);
        if ($sort == self::SORT_COURSE) {
            $params['actsort'] = $sort;
        }
