...
class progress_report_subreport_category{
     ...
     */
    const SETTING_ENABLED                       = 'report_grade_progress_cat_enabled';
    /**
     * consumed by grade_get_setting
     * @const string SETTING_WEIGHT
     */
    const SETTING_WEIGHT                        = 'report_grade_progress_cat_weight';
    /**
     * consumed by grade_get_setting
     * @const string SETTING_WEIGHT
     */
    const SETTING_DEPTH                         = 'report_grade_progress_cat_depth';
    ...
    /**
     * array of courses which are set to available for this report
     * @var array $courses
     */
    protected $availablecourses = array();
    ...
    public function __construct($userid, $courseid, $context, $course, $params) {
        ...
        $this->initialize_available_courses();
    ...
    /**
     * Builds $this->availablecourses
     */
    protected function initialize_available_courses(){

        global $DB;

        //include all courses by default
        if ($courses = $DB->get_records_sql('SELECT id FROM {course} WHERE id != 1', array())) {
            
            foreach ($courses as $course) {
                if (self::is_course_allowed($this->courseid, $course->id)) {
                    $this->availablecourses[$course->id] = $course->id;
                }
            }
        }
    }
    ...
    protected function initialize_courses(){
        ...
        foreach ($this->categories as $category) {
            ...
            foreach ($rs as $course) {
                if (array_key_exists($course->id, $this->availablecourses)) { //if course is available, add to array of courses considered in report.
    ...
    protected function initialize_overall_grades() {
        ...
        foreach ($this->courses as $course) {
            //if enrolled, clone info for dummy user
            if ($course['enrolled']) {
    ...
    public function get_subreport_content() {
        ...
        $settings = array();
        $settings['report_grade_progress_cat_show_submitted'] = self::get_category_setting($this->courseid, 'report_grade_progress_cat_show_submitted', 0);
        $settings['report_grade_progress_cat_show_available'] = self::get_category_setting($this->courseid, 'report_grade_progress_cat_show_available', 0);
        ...
        foreach ($this->courses as $course) {
            ...
            if ($course['access']) {
                if ($course['enrolled']) {
                    ...
                    if ($settings['report_grade_progress_cat_show_submitted']) {
                    ...
                    if ($settings['report_grade_progress_cat_show_available']) {
    ...
    /**
     * Returns a category subreport setting
     * @return int value
     */
    static public function get_category_setting($courseid, $name, $default) {
        global $CFG;

        $defaultvalue = isset($CFG->$name)? $CFG->$name : $default;
        $setting = grade_get_setting($courseid, $name, $defaultvalue);
        if (!is_null($setting)) {
            return $setting;
        }
        return $default;
    ...
    /**
     * Returns whether or not course is to be considered for category report
     * @return boolean
     */
    static public function is_course_allowed($thiscourseid,$courseid) {

        $setting = grade_get_setting($thiscourseid, 'report_grade_progress_cat_show_'.$courseid, 0);
        if (!is_null($setting)) {
            return $setting;
        }
        return 0;
